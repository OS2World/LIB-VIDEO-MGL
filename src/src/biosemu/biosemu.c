...
/****************************************************************************
PARAMETERS:
shared		- True if running in a shared environment
	...
	BE_VGAInfo *info)
{
	...
	if (shared)
		M.mem_base = (unsigned long)PM_mallocShared(memSize);
	else
...
/****************************************************************************
REMARKS:
This function is called by shell drivers when a new process attaches itself
to the shell driver, and the shell driver requires all global shared data
to be mapped to the process. This function is only useful if the driver
was originally loaded with the 'shared' parameter set to TRUE.
****************************************************************************/
void PMAPI BE_mapShared(void)
{
	PM_mapShared((void*)M.mem_base);
	PM_mapShared((void*)_BE_env.biosmem_base);
	PM_mapToProcess((void*)_BE_env.busmem_base,0x5FFFF);
	PCI_mapShared();
}
