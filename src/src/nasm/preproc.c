...
static int do_directive (Token *tline) 
{
	...
	if (stricmp(tline->text,"flat") == 0) {
	    /* All subsequent ARG directives are for a 32-bit stack */
	    StackSize = 4;
	    StackPointer = "ebp";
	    ArgOffset = 8;
	    LocalOffset = 4;
	} else if (stricmp(tline->text,"large") == 0) {
	    ...
	    ArgOffset = 4;
	    LocalOffset = 2;
	} else if (stricmp(tline->text,"small") == 0) {
		/* All subsequent ARG directives are for a 16-bit stack,
	...
	do {
	    ...
	    tt = expand_smacro(tt);
	    if (stricmp(tt->text,"byte") == 0) {
		size = MAX(StackSize,1);
	    } else if (stricmp(tt->text,"word") == 0) {
		size = MAX(StackSize,2);
	    } else if (stricmp(tt->text,"dword") == 0) {
		size = MAX(StackSize,4);
	    } else if (stricmp(tt->text,"qword") == 0) {
		size = MAX(StackSize,8);
	    } else if (stricmp(tt->text,"tword") == 0) {
