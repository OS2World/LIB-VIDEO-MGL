...
int main(int argc, char *argv[])
{
   ...
   printf("Packinst- version 2.0 (c) 1999 DBSoft\n\n");
   if(argc!=2)
      {
   ...
   if((installfile = fopen("sddinst.exe", "wb"))==NULL)
      {
         printf("Error opening sddinst.exe for writing!\n");
         exit(3);
      }
#ifndef SDDINST
   ...
   for(z=0;z<INSTALLER_PACKAGE_COUNT;z++)
      {
         ...
         fprintf(installfile, "DBSOFT-ACE%d%d-%d~", z, packagefiles[z], blah.st_size);
