...
main(void)
{
	...
	scan_pci();
	if (!CurrentPci && !CONFIG_ACTIVE_DEVICE)
		exit (1);
	...
	while (CurrentPci) {
		...
		if (!((mapPciRom() && chksum((CARD8*)V_BIOS))
		...
		if (SAVE_BIOS) save_bios_to_file();
	...
	if (!Active_is_Pci && CONFIG_ACTIVE_DEVICE) {
		pciVideoDisable();
