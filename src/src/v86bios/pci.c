...
static CARD8 interpretConfigSpace(CARD32 *reg,CARD32 *pcibuses, int busidx,
				  CARD8 dev, CARD8 func);
static CARD32 findBIOSMap(CARD32 *biosSize);
static void restoreMem(void);
...
scan_pci(void)
{
    ...
    if (configtype == 1) {
		...
		do {
			...
			for (cardnum = 0; cardnum < MAX_DEV_PER_VENDOR_CFG1; cardnum++) {
				...
				do {
					...
					if (!checkSlotCfg1(pcibuses[busidx],cardnum,func))
						break;
					readConfigSpaceCfg1(pcibuses[busidx],cardnum,func,reg);
		    
					func = interpretConfigSpace(reg,pcibuses,busidx,
												cardnum,func);
		...
		do {
			for (slot=0xc0; slot<0xd0; i++) {
				if (!checkSlotCfg2(pcibuses[busidx],slot))
					break;
				readConfigSpaceCfg2(pcibuses[busidx],slot,reg);
		
				interpretConfigSpace(reg,pcibuses,busidx,
									 slot,0);
				if (idx++ > MAX_PCI_DEVICES)
					continue;
			}
    ...
    pciMaxBus = numbus - 1;
    P_printf("Max buses in system: %i\n",pciMaxBus + 1);
