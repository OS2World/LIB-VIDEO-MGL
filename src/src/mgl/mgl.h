...
typedef struct {
	...
	ushort	xRes;				/* X resolution for display mode		*/
	ushort	yRes;				/* Y resolution for display mode		*/
...
typedef	struct {
	...
	PE_MODULE	*hModBPD;		/* Handle to binary portable DLL		*/
	PM_MODULE	hModOS;			/* Handle to OS speicific DLL			*/
...
typedef struct {
	...
	int					whichOpenGL;	/* User OpenGL driver selection	*/
	int					cntOpenGL;		/* Current OpenGL selection		*/
	void				*stateBuf;		/* Console state buffer			*/
	void				*gaInfo;		/* Pointer to Nucleus info		*/
	GA_devCtx			*gaPtr;			/* Pointer to Nucleus driver	*/
	REF2D_driver		*ref2d;			/* Nucleus reference rasteriser	*/
	PE_MODULE			*hModRef2d;		/* Nucleus ref2d module handle	*/
	GA_buffer			drawBuf;		/* Currently active draw buffer	*/
	LIST				*dispDCList;	/* List of display contexts		*/
	LIST				*offDCList;		/* List of offscreen contexts	*/
	LIST				*offBufList;	/* List of offscreen buffers	*/
	LIST				*overDCList;	/* List of overlay contexts		*/
	long				linearMin;		/* Minimum offscreen address	*/
	long				linearCur;		/* Current offscreen address	*/
	long				linearMax;		/* Maximum offscreen address	*/
	ibool				compacted;		/* True if heap is compacted	*/
	region_t			*freeSpace;		/* Free offscreen memory region	*/
	} device_entry;
...
typedef struct {
	...
	ulong			dwOffset;				/* Offset within file			*/
	char			numFonts;				/* Number of fonts in lib		*/
	char			*fontSizes;				/* Font sizes in lib			*/
	ulong			*offset;				/* Offsets of fonts in library	*/
	ulong			*length;				/* Length of font in library	*/
...
typedef struct {
	...
	TT_Instance		instance;				/* Font instance handle			*/
	char			numFonts;				/* Number of fonts in lib		*/
...
#define	MAKE_ACTIVE_DEVICE(dc)											\
	if ((_MGL_cntDevice & ~MM_MODE_MIXED) != dc->v->d.device)   		\
		dc->v->d.setActiveDevice(dc->v->d.device);

#define RESTORE_ACTIVE_DEVICE()											\
	if (dc->v->d.setActiveDevice)										\
		dc->v->d.setActiveDevice(_MGL_cntDevice);

#define	MAKE_CURRENT_ACTIVE_DEVICE(dc)									\
	if (!(_MGL_cntDevice & MM_MODE_MIXED)) {                            \
		if (_MGL_cntDevice != dc->v->d.device)                          \
			dc->v->d.setActiveDevice(_MGL_cntDevice = dc->v->d.device); \
		}
...
{ 														\
	if (dc != &DC && dc->currentLock++ == 0)  			\
		dc->r.makeCurrent(dc,partial);					\
}

#define	RESTORE_HARDWARE(dc,partial)					\
{ 														\
	if (dc != &DC && dc->currentLock-- == 1) 			\
		_MGL_dcPtr->r.makeCurrent(_MGL_dcPtr,partial);	\
}

/* Macro to type cast a pointer to an MGL pixel format strucure to a
 * Nucleus pixel format structure pointers. The structures are identical,
 ...
 * Nucleus palette entry pointer. The structures are identical, so we
 * simply use a type cast.
 */

#define	GAPAL(pal)		(GA_palette*)(pal)

/* Macros to convert to Nucleus style patterns. MGL patterns are the same
 * structures, so we can just use a cast to convert between them.
 */
...
#ifdef	__WINDOWS__
#define	MGL_WINDOWEDNAME	"wingdi.drv"
#endif
...
extern char	 			_MGL_path[_MAX_PATH];
...
extern uchar 			_MGL_translate[256];
extern int				_MGL_surfLock;
extern void * 			(*__MGL_malloc)(size_t size);
extern void 			(*__MGL_free)(void *p);
...
/* Internal drivers; these will become BPD's and DLL's eventually! */

//#ifdef	DEBUG
extern drivertype 		GLDIRECT_driver;
extern drivertype 		OPENGL_driver;
extern drivertype 		NUCLEUS_driver;
extern drivertype 		DDRAW_driver;
...
#ifdef	__WINDOWS__
extern drivertype 		WINGDI_driver;
#endif
...
void 	_MGL_freeSurface(MGLDC *dc);
...
void 	MGLAPI _MGL_clipPixel(long x,long y);
...
void	__MGL_ellipseArcEngine(int left,int top,int A,int B,int startAngle,int endAngle,arc_coords_t *ac,void (MGLAPIP plotPoint)(long x,long y));
...
void	_MGL_convexPolygon(int count,fxpoint_t *vArray,int vinc,fix32_t xOffset,fix32_t yOffset);
