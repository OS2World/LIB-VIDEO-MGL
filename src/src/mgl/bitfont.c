	...
	uchar *buffer)
{
	...
	if (!MGL_sectRect(DC.intClipRect,d,&r))
		return;
		...
		BEGIN_CLIP_REGION(clip,DC.intClipRegion);
			if (MGL_sectRect(clip,d,&r)) {
				if (!MGL_equalRect(d,r)) {
					...
					DC.r.ClipMonoImage(x,y,width,height,byteWidth,buffer,DC.a.backMode,
						DC.intClipRect.left,DC.intClipRect.top,
						DC.intClipRect.right,DC.intClipRect.bottom);
					}
				else {
					/* Draw it unclipped for maximum speed */
					DC.r.PutMonoImage(x,y,width,height,byteWidth,buffer,DC.a.backMode);
					}
				}
		END_CLIP_REGION;
		}
	...
	if (DC.a.clipRegion) {
		...
		BEGIN_CLIP_REGION(clip,DC.intClipRegion);
			if (MGL_sectRect(clip,d,&r)) {
				if (!MGL_equalRect(d,r)) {
					/* Draw it clipped to the clip rectangle */
					DC.r.ClipMonoImage(x,y,width,height,byteWidth,buffer,true,
						DC.intClipRect.left,DC.intClipRect.top,
						DC.intClipRect.right,DC.intClipRect.bottom);
					}
				else {
					/* Draw it unclipped for maximum speed */
					DC.r.PutMonoImage(x,y,width,height,byteWidth,buffer,true);
					}
				}
		END_CLIP_REGION;
		}
	...
	int offset)
{
	...
	if (DC.a.fontBlendMode == MGL_AA_RGBBLEND && DC.a.backMode == MGL_TRANSPARENT_BACKGROUND) {
		...
		DC.r.SetSrcBlendFunc(MGL_BLEND_CONSTANTALPHA);
		DC.r.SetDstBlendFunc(MGL_BLEND_ONEMINUSCONSTANTALPHA);
	...
	bitmap_font *font)
{
	...
	idx = TT_Char_Index(lib->charMap,ch+font->startGlyph);
	if (idx == 0) {
        int i;
        i = '.' - font->startGlyph;
        if ((i >= font->numGlyphs) ||
    	...
    	if (!font->valid[ch]) {
			...
			if (TT_Load_Glyph(lib->instance,glyph, idx, TTLOAD_DEFAULT))
