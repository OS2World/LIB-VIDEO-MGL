...
#include "mgldd.h"
#ifndef	__WIN386__
#include "drivers/win/ddraw32.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI DDRAW32_driver = {
	IDENT_FLAG,
	MGL_DDRAW32NAME,
	MGL_DDRAW32NAME,
...
ibool MGLAPI DDRAW32_detect(int id,int force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		DDRAW32_detect
...
****************************************************************************/
{
	if (force >= grDDRAW && DDRAW_detect(driver,&DDRAW32_driver.driverId)) {
		/* Fill in information here for the available video modes by
		 * querying the DDRAW.DRV for the information.
		 */
		DDRAW_useMode(modes,grSVGA_320x200x4G,id,&DDRAW32_modes[0]);
		DDRAW_useMode(modes,grSVGA_320x240x4G,id,&DDRAW32_modes[1]);
		DDRAW_useMode(modes,grSVGA_320x400x4G,id,&DDRAW32_modes[2]);
		DDRAW_useMode(modes,grSVGA_320x480x4G,id,&DDRAW32_modes[3]);
		DDRAW_useMode(modes,grSVGA_400x300x4G,id,&DDRAW32_modes[4]);
		DDRAW_useMode(modes,grSVGA_512x384x4G,id,&DDRAW32_modes[5]);
		DDRAW_useMode(modes,grSVGA_640x350x4G,id,&DDRAW32_modes[6]);
		DDRAW_useMode(modes,grSVGA_640x400x4G,id,&DDRAW32_modes[7]);
		DDRAW_useMode(modes,grSVGA_640x480x4G,id,&DDRAW32_modes[8]);
		DDRAW_useMode(modes,grSVGA_800x600x4G,id,&DDRAW32_modes[9]);
		DDRAW_useMode(modes,grSVGA_1024x768x4G,id,&DDRAW32_modes[10]);
		DDRAW_useMode(modes,grSVGA_1152x864x4G,id,&DDRAW32_modes[11]);
		DDRAW_useMode(modes,grSVGA_1280x960x4G,id,&DDRAW32_modes[12]);
		DDRAW_useMode(modes,grSVGA_1280x1024x4G,id,&DDRAW32_modes[13]);
		DDRAW_useMode(modes,grSVGA_1600x1200x4G,id,&DDRAW32_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
PUBLIC void DDRAW32_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		DDRAW32_initInternal
* Parameters:	dc	- Device context.
...
ibool MGLAPI DDRAW32_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		DDRAW32_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	dc->mi = DDRAW32_modes[modeId - grSVGA_320x200x4G];
	if (g_state == NULL) {
