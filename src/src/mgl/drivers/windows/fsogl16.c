...
* Environment:	Any
*
* Description:	Main 16 bit OpenGL device driver file.
*
****************************************************************************/

#include "mgldd.h"
#include "drivers/opengl/fsogl16.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI FSOGL16_driver = {
	IDENT_FLAG,
	MGL_FSOGL16NAME,
	MGL_FSOGL16NAME,
	...
	grOPENGL_MGL,
	FSOPENGL16_detect,
	FSOPENGL16_initDriver,
	};

/* List of mode specific device context information */

PRIVATE gmode_t FSOPENGL15_modes[] = {
...
PRIVATE gmode_t FSOPENGL16_modes[] = {
...
ibool MGLAPI FSOPENGL16_detect(int id,int *force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		FSOPENGL16_detect
...
****************************************************************************/
{
	if (*force >= grOPENGL_MGL && FSOPENGL_detect(driver,&FSOGL16_driver.driverId,FSOGL16_driver.realname)) {
		/* Fill in information here for the available video modes by
		 * querying the OPENGL.DRV for the information.
		 */
		FSOPENGL_useMode(modes,grSVGA_320x200x32k,id,&FSOPENGL15_modes[0]);
		FSOPENGL_useMode(modes,grSVGA_320x240x32k,id,&FSOPENGL15_modes[1]);
		FSOPENGL_useMode(modes,grSVGA_320x400x32k,id,&FSOPENGL15_modes[2]);
		FSOPENGL_useMode(modes,grSVGA_320x480x32k,id,&FSOPENGL15_modes[3]);
		FSOPENGL_useMode(modes,grSVGA_400x300x32k,id,&FSOPENGL15_modes[4]);
		FSOPENGL_useMode(modes,grSVGA_512x384x32k,id,&FSOPENGL15_modes[5]);
		FSOPENGL_useMode(modes,grSVGA_640x350x32k,id,&FSOPENGL15_modes[6]);
		FSOPENGL_useMode(modes,grSVGA_640x400x32k,id,&FSOPENGL15_modes[7]);
		FSOPENGL_useMode(modes,grSVGA_640x480x32k,id,&FSOPENGL15_modes[8]);
		FSOPENGL_useMode(modes,grSVGA_800x600x32k,id,&FSOPENGL15_modes[9]);
		FSOPENGL_useMode(modes,grSVGA_1024x768x32k,id,&FSOPENGL15_modes[10]);
		FSOPENGL_useMode(modes,grSVGA_1152x864x32k,id,&FSOPENGL15_modes[11]);
		FSOPENGL_useMode(modes,grSVGA_1280x960x32k,id,&FSOPENGL15_modes[12]);
		FSOPENGL_useMode(modes,grSVGA_1280x1024x32k,id,&FSOPENGL15_modes[13]);
		FSOPENGL_useMode(modes,grSVGA_1600x1200x32k,id,&FSOPENGL15_modes[14]);

		FSOPENGL_useMode(modes,grSVGA_320x200x64k,id,&FSOPENGL16_modes[0]);
		FSOPENGL_useMode(modes,grSVGA_320x240x64k,id,&FSOPENGL16_modes[1]);
		FSOPENGL_useMode(modes,grSVGA_320x400x64k,id,&FSOPENGL16_modes[2]);
		FSOPENGL_useMode(modes,grSVGA_320x480x64k,id,&FSOPENGL16_modes[3]);
		FSOPENGL_useMode(modes,grSVGA_400x300x64k,id,&FSOPENGL16_modes[4]);
		FSOPENGL_useMode(modes,grSVGA_512x384x64k,id,&FSOPENGL16_modes[5]);
		FSOPENGL_useMode(modes,grSVGA_640x350x64k,id,&FSOPENGL16_modes[6]);
		FSOPENGL_useMode(modes,grSVGA_640x400x64k,id,&FSOPENGL16_modes[7]);
		FSOPENGL_useMode(modes,grSVGA_640x480x64k,id,&FSOPENGL16_modes[8]);
		FSOPENGL_useMode(modes,grSVGA_800x600x64k,id,&FSOPENGL16_modes[9]);
		FSOPENGL_useMode(modes,grSVGA_1024x768x64k,id,&FSOPENGL16_modes[10]);
		FSOPENGL_useMode(modes,grSVGA_1152x864x64k,id,&FSOPENGL16_modes[11]);
		FSOPENGL_useMode(modes,grSVGA_1280x960x64k,id,&FSOPENGL16_modes[12]);
		FSOPENGL_useMode(modes,grSVGA_1280x1024x64k,id,&FSOPENGL16_modes[13]);
		FSOPENGL_useMode(modes,grSVGA_1600x1200x64k,id,&FSOPENGL16_modes[14]);
		return true;
		}
	mode = mode;
	return false;
...
PUBLIC void FSOPENGL16_initInternal(MGLDC *dc)
/****************************************************************************
*
* Function:		FSOPENGL16_initInternal
* Parameters:	dc	- Device context.
...
ibool MGLAPI FSOPENGL16_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		FSOPENGL16_initDriver
* Parameters:	dc	- Device context.
* Returns:		True if the device was correctly initialised.
*
...
****************************************************************************/
{
	...
	if (modeId >= grSVGA_320x200x64k)
		dc->mi = FSOPENGL16_modes[modeId - grSVGA_320x200x64k];
	else
		dc->mi = FSOPENGL15_modes[modeId - grSVGA_320x200x32k];
	if (g_state == NULL) {
