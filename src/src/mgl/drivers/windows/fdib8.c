...
#include "mgldd.h"
#include "drivers/win/fdib8.h"

/*---------------------------- Driver Tables -----------------------------*/

drivertype	_VARAPI FULLDIB8_driver = {
	IDENT_FLAG,
	MGL_FULLDIB8NAME,
	MGL_FULLDIB8NAME,
...
PRIVATE gmode_t FULLDIB8_modes[] = {
...
ibool MGLAPI FULLDIB8_detect(int id,int force,int *driver,int *mode,
	modetab modes)
/****************************************************************************
*
* Function:		FULLDIB8_detect
...
****************************************************************************/
{
	if (force >= grFULLDIB && FULLDIB_detect()) {
		 ...
		 */
		FULLDIB_useMode(modes,grSVGA_640x480x256,id,&FULLDIB8_modes[0]);
		FULLDIB_useMode(modes,grSVGA_800x600x256,id,&FULLDIB8_modes[1]);
		FULLDIB_useMode(modes,grSVGA_1024x768x256,id,&FULLDIB8_modes[2]);
		FULLDIB_useMode(modes,grSVGA_1152x864x256,id,&FULLDIB8_modes[3]);
		FULLDIB_useMode(modes,grSVGA_1280x960x256,id,&FULLDIB8_modes[4]);
		FULLDIB_useMode(modes,grSVGA_1280x1024x256,id,&FULLDIB8_modes[5]);
		FULLDIB_useMode(modes,grSVGA_1600x1200x256,id,&FULLDIB8_modes[6]);
		return true;
		}
	mode = mode;
	return false;
...
ibool MGLAPI FULLDIB8_initDriver(MGLDC *dc,int driverId,int modeId,ulong hwnd,
	int virtualX,int virtualY,int numBuffers,ibool stereo,int refreshRate)
/****************************************************************************
*
* Function:		FULLDIB8_initDriver
* Parameters:	dc	- Device context.
...
****************************************************************************/
{
	...
	if (stereo)
    	return false;
	dc->mi = FULLDIB8_modes[modeId - grSVGA_640x480x256];
	if (g_state == NULL) {
