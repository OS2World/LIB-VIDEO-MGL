...
drivertype PACKED24_driver = {
	MGL_PACKED24NAME,
	"SciTech Nucleus 24bpp Packed Pixel Driver",
	"\r\n\r\nVersion " DRIVER_VERSION_STR " - " __DATE__ "\r\n"
	"Copyright (C) 1996-1999 SciTech Software\r\n",
	DRIVER_MAJOR_VERSION,
	DRIVER_MINOR_VERSION,
	PACKED24_createInstance,
	NULL,
	PACKED24_initDriver,
	PACKED24_destroyInstance,
	};
...
****************************************************************************/
void * MGLAPI PACKED24_createInstance(void)
{
	return MGL_calloc(1,sizeof(PACKED24_state));
}
...
****************************************************************************/
static void MGLAPI PACKED24_destroyDC(
	MGLDC *dc)
{
...
function is called.
****************************************************************************/
ibool MGLAPI PACKED24_initDriver(
	void *data,
	MGLDC *dc,
	modeent *mode,
	ulong hwnd,
...
{
	PACKED24_state		*state = (PACKED24_state*)data;
	gmode_t				*mi = &dc->mi;
	REF2D_driver		*ref2d;
#ifndef	TEST_HARNESS
	GA_buffer			drawBuf;
	 ...
	 */
	dc->v = (drv_vec*)state;
	dc->v->m.destroy = PACKED24_destroyDC;
	if (dc->v->m.refCount++ == 0) {
...
****************************************************************************/
void MGLAPI PACKED24_destroyInstance(
	void *data)
{
	if (data)
		MGL_free(data);
