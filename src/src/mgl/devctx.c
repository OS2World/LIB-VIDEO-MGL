	...
	int page)
{
	if (_MGL_isFullscreenDevice(dc)) {
		...
		if (dc == _MGL_dcPtr) {
			...
			DC.originOffset = dc->originOffset;
			DC.activePage = dc->activePage;
			DC.activeStartY = dc->activeStartY;
			DC.offscreenStartY = dc->offscreenStartY;
			DC.offscreenY = dc->offscreenY;
			}
		}
}
...
ibool MGLAPI MGL_vSync(
	MGLDC *dc)
{
	if (_MGL_isFullscreenDevice(dc) && dc->v->d.vSync) {
		dc->v->d.vSync(dc);
		return true;
		}
	return false;
...
int	MGLAPI MGL_isVSync(
	MGLDC *dc)
{
	if (_MGL_isFullscreenDevice(dc) && dc->v->d.isVSync)
...
int MGLAPI MGL_getCurrentScanLine(
	MGLDC *dc)
{
	if (_MGL_isFullscreenDevice(dc) && dc->v->d.getCurrentScanLine)
