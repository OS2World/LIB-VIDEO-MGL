...
uchar _MGL_translate[256];		/* Palette translation vector		*/
...
numColors	- Number of gamma entries to set
startIndex	- Starting index of first gamma entry to set
vSync		- True if routine should sync to vertical retrace, false if not.
	...
	ibool vSync)
{
	if (dc->r.setGammaRamp) {
		dc->r.setGammaRamp(dc,gamma,num,index,vSync);
		return true;
		}
	return false;
}
...
numColors	- Number of gamma entries to retrieve
startIndex	- Starting gamma entry index to retrieve
