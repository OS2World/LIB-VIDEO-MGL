	...
	const char *str)
{
	...
	if (str) {
		switch (DC.a.ts.font->fontType & MGL_FONTTYPEMASK) {
			...
			case MGL_PROPFONT:
				while (*str)
					width += __MGL_charWidth((uchar)*str++);
				break;
			default:
				return 0;
			}
...
void MGLAPI MGL_getCharMetrics(
	...
	metrics_t *m)
{
	__MGL_getCharMetrics((uchar)ch,m);
}
	...
	const char *str)
{
	...
	switch (DC.a.ts.font->fontType & MGL_FONTTYPEMASK) {
		case MGL_VECTORFONT:
			_MGL_drawStrVec(x+DC.a.viewPort.left,y+DC.a.viewPort.top,str);
			break;
		case MGL_FIXEDFONT:
		case MGL_PROPFONT:
			_MGL_drawStrBitmap(x+DC.a.viewPort.left,y+DC.a.viewPort.top,str);
			break;
		}
}
