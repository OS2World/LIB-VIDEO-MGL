...
coordinates. We use the internal buffer to scan convert the ellipse.
****************************************************************************/
static void _MGL_fatPenEllipse(
	int left,
	int top,
	int A,
	int B,
	...
	rect_t extentRect)
{
	...
	if (DC.a.clipRegion) {
		...
		clipped = MGL_sectRegion(ellipse,DC.a.clipRegion);
		if (!MGL_emptyRect(clipped->rect))
			__MGL_drawRegion(DC.a.viewPort.left, DC.a.viewPort.top, clipped);
		MGL_freeRegion(pen);
		...
		if (DC.a.penStyle == MGL_BITMAP_SOLID && DC.a.penWidth == 0 && DC.a.penHeight == 0) {
			if (clipit) {
				DC.r.ClipEllipse(extentRect.left + DC.a.viewPort.left,
					extentRect.top + DC.a.viewPort.top,A,B,
					DC.intClipRect.left, DC.intClipRect.top,
					DC.intClipRect.right, DC.intClipRect.bottom);
				}
			else {
				DC.r.DrawEllipse(extentRect.left + DC.a.viewPort.left,
					extentRect.top + DC.a.viewPort.top,A,B);
				}
			}
		else {
			_MGL_fatPenEllipse(extentRect.left + DC.a.viewPort.left,
	...
	rect_t extentRect)
{
	...
	if (DC.a.clipRegion) {
		...
		_MGL_fillEllipse(extentRect.left + DC.a.viewPort.left,
			extentRect.top + DC.a.viewPort.top,A,B,clipit);
		}
}
