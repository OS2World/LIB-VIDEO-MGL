...
/****************************************************************************
REMARKS:
This function is called after the DLL's have been loaded from disk and
initialised, but before any driver is actually loaded.
****************************************************************************/
void NAPI _GA_postLoadDriver(void)
{
	/* Overload the GA_loadDriver function so we can load it when the
	 * Windows GUI is already active and in graphics mode.
	 */
	ORG_GA_loadDriver 							= _GA_exports.GA_loadDriver;
	_GA_exports.GA_loadDriver 					= _GA_loadDriver;

	/* Now override the stereo functions exported from the library to
	 * call into the VxD instead of the user land functions, so that we
	 * can properly implemente stereo flipping at ring 0 in our helper
	 * device driver.
	 */
	_GA_exports.GA_softStereoInit 				= _GA_softStereoInit;
	_GA_exports.GA_softStereoOn 				= _GA_softStereoOn;
	_GA_exports.GA_softStereoScheduleFlip		= _GA_softStereoScheduleFlip;
	_GA_exports.GA_softStereoGetFlipStatus		= _GA_softStereoGetFlipStatus;
	_GA_exports.GA_softStereoWaitTillFlipped	= _GA_softStereoWaitTillFlipped;
	_GA_exports.GA_softStereoOff				= _GA_softStereoOff;
	_GA_exports.GA_softStereoExit				= _GA_softStereoExit;
}
