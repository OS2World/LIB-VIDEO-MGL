	...
	ibool accurate)
{
	...
	do {
		...
		ticks = _CPU_mulDiv(ticks,1000000,count_freq.low);
		total_ticks += ticks;
		total_cycles += cycles;
		if ((ticks % count_freq.low) > (count_freq.low/2))
			ticks++;			/* Round up if necessary 					*/
	...
	ibool accurate)
{
	...
	static 	ulong known_speeds[] = {
		650,600,550,500,450,433,400,350,333,300,266,233,200,166,150,133,120,100,90,75,66,60,50,33,20,0,
		};
	if (CPU_haveRDTSC()) {
		cpuSpeed = GetRDTSCCpuSpeed(accurate);
		}
	else {
