...
static void PMAPI lockPMHandlers(void)
{
	...
	if (!locked) {
		...
		stat |= !PM_lockDataPages(&_PM_DMADataStart,(int)&_PM_DMADataEnd - (int)&_PM_DMADataStart);
		stat |= !PM_lockCodePages((__codePtr)_PM_DMACodeStart,(int)_PM_DMACodeEnd-(int)_PM_DMACodeStart);
		if (stat) {
	...
	PM_irqHandler ih)
{
	...
	if ((handle = malloc(sizeof(_PM_IRQHandle) + thunkSize)) == NULL)
		return NULL;
	handle->IRQ = IRQ;
