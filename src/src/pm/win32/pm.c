...
extern ushort	_PM_gdt;			/* GDT for our Ring-0 Call Gate		*/
...
static char *szMachineNameKeyNT = "System\\CurrentControlSet001\\control\\ComputerName\\ActiveComputerName";
...
void PMAPI PM_init(void)
{
	...
	/* Determine if we are running under Windows NT or not */
	_PM_haveWinNT = false;
	 ...
	 */
	if (!inited) {
		_PM_hDevice = CreateFile(SDDHELP_VXD_MODULE, 0,0,0, CREATE_NEW, FILE_FLAG_DELETE_ON_CLOSE, 0);
		...
		if (_PM_hDevice == INVALID_HANDLE_VALUE) {
			/* The VxD was not staticly loaded, so try creating a file handle
			 * to a dynamic version of the VxD if possible.
			 */
			_PM_hDevice = CreateFile(PMHELP_VXD_NAME, 0,0,0, CREATE_NEW, FILE_FLAG_DELETE_ON_CLOSE, 0);
			if (_PM_hDevice == INVALID_HANDLE_VALUE)
		...
		/* Obtain the 32->16 callgate from the VxD to enable IOPL */
		if (!DeviceIoControl(_PM_hDevice, PMHELP_GETCALLGATE32, NULL, 0, outBuf, sizeof(outBuf), &count, NULL))
			PM_fatalError("Unable to obtain call gate selector!");
		_PM_gdt = (ushort)outBuf[0];
		...
		if (!DeviceIoControl(_PM_hDevice, PMHELP_SETCNTPATH32, inBuf, sizeof(inBuf), NULL, 0, NULL, NULL))
	...
	void *ptr)
{
	...
	inBuf[0] = (ulong)ptr;
	if (DeviceIoControl(_PM_hDevice, PMHELP_MAPSHARED32, inBuf, sizeof(inBuf),
			outBuf, sizeof(outBuf), &count, NULL))
		return outBuf[0];
	return -1;
}
