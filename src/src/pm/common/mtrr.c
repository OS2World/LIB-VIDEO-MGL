...
static int MTRR_haveWriteCombine(void)
{
	...
	switch (cpuFamily) {
		...
		case CPU_Intel:
			...
			return (config & (1 << 10));
// TODO: This appears to be busted at the moment!!
//
//		case CPU_Cyrix:
	...
	int *type)
{
	...
	((uchar*)base)[2]  = _MTRR_getCx86(arr+1);
	((uchar*)base)[1]  = _MTRR_getCx86(arr+2);
	rcr = _MTRR_getCx86(CX86_RCR_BASE + reg);
	MTRR_endUpdate(&c);
	...
	int type)
{
	...
	MTRR_beginUpdate(&c);
	_MTRR_setCx86(arr,    ((uchar*)&base)[3]);
	_MTRR_setCx86(arr+1,  ((uchar*)&base)[2]);
	_MTRR_setCx86(arr+2, (((uchar*)&base)[1]) | arr_size);
	_MTRR_setCx86(CX86_RCR_BASE + reg, arr_type);
	MTRR_endUpdate(&c);
}
