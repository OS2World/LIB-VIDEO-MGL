...
static ulong	keyModifiers = 0;		/* Current keyboard modifiers	*/
static uchar    keyTable[128];			/* Table of key up/down flags	*/
static ibool	allowLEDS = true;		/* True if LEDS should change	*/
	...
	event_t *evt)
{
	...
	if (evt->modifiers & EVT_NUMLOCK) {
		if ((ascii = translateScan(scan,codePage->numPad,codePage->numPadLen)) != 0) {
			evt->message |= ascii;
			return;
			}
		}
	if (evt->modifiers & EVT_CTRLSTATE) {
		evt->message |= translateScan(scan,codePage->ctrl,codePage->ctrlLen);
		return;
		}
	if (evt->modifiers & EVT_CAPSLOCK) {
		if (evt->modifiers & EVT_SHIFTKEY) {
			if ((ascii = translateScan(scan,codePage->shiftCaps,codePage->shiftCapsLen)) != 0) {
				evt->message |= ascii;
				return;
				}
			}
		else {
			if ((ascii = translateScan(scan,codePage->caps,codePage->capsLen)) != 0) {
				evt->message |= ascii;
				return;
				}
			}
	...
	if (evt->modifiers & EVT_SHIFTKEY) {
		if ((ascii = translateScan(scan,codePage->shift,codePage->shiftLen)) != 0) {
			evt->message |= ascii;
			return;
			}
		}
	evt->message |= translateScan(scan,codePage->normal,codePage->normalLen);
}
	...
	if (pauseLoop) {
		...
		if (what != EVT_KEYREPEAT) {
			switch (scan) {
				case KB_capsLock:
					if (what == EVT_KEYDOWN)
						keyModifiers ^= EVT_CAPSLOCK;
					setLEDS(keyModifiers);
					break;
				case KB_numLock:
					if (what == EVT_KEYDOWN)
						keyModifiers ^= EVT_NUMLOCK;
					setLEDS(keyModifiers);
					break;
				case KB_scrollLock:
					if (what == EVT_KEYDOWN)
						keyModifiers ^= EVT_SCROLLLOCK;
					setLEDS(keyModifiers);
					break;
				case KB_leftShift:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_LEFTSHIFT;
					else
						keyModifiers |= EVT_LEFTSHIFT;
					break;
				case KB_rightShift:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_RIGHTSHIFT;
					else
						keyModifiers |= EVT_RIGHTSHIFT;
					break;
				case KB_leftCtrl:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_LEFTCTRL;
					else
						keyModifiers |= EVT_LEFTCTRL;
					break;
				case KB_rightCtrl:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_RIGHTCTRL;
					else
						keyModifiers |= EVT_RIGHTCTRL;
					break;
				case KB_leftAlt:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_LEFTALT;
					else
						keyModifiers |= EVT_LEFTALT;
					break;
				case KB_rightAlt:
					if (what == EVT_KEYUP)
						keyModifiers &= ~EVT_RIGHTALT;
					else
						keyModifiers |= EVT_RIGHTALT;
					break;
				}
