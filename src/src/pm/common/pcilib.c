...
/****************************************************************************
REMARKS:
This function maps in any shared memory needed by the PCI BIOS functions.
****************************************************************************/
void _ASMAPI PCI_mapShared(void)
{
	PM_mapToProcess(BIOSImage,BIOS_LIMIT);
}
	...
	int index)
{
	 ...
	 */
	for (i = duplicate = 0; i < index; i++) {
		if (info[i].slot.p.Bus == info[index].slot.p.Bus &&
			info[i].slot.p.Device == info[index].slot.p.Device &&
			info[i].DeviceID == info[index].DeviceID) {
			duplicate = 1;
			break;
			}
		}
			...
			for (device = 0; device < 32; device++) {
				...
				for (function = 0; function < 8; function++) {
					...
					if (PM_inpd(0xCFC) != 0xFFFFFFFFUL) {
						...
						if (!CheckDuplicate(info,numFound)) {
							if (++numFound >= maxDevices)
								break;
							}
						}
					if (numFound >= maxDevices)
	...
	int maxDevices)
{
	 ...
	 */
	if ((numFound = PCI_enumerateMech1(info,maxDevices)) == 0) {
		if ((numFound = PCI_enumerateMech2(info,maxDevices)) == 0) {
			if ((numFound = PCI_enumerateBIOS(info,maxDevices)) == 0)
				return 0;
			}
		}

	/* Now measure the size of all PCI base address registers */
	for (i = 0; i < numFound; i++) {
		size = (ulong*)&(info[i].BaseAddress10Len);
		for (j = 0x10; j <= 0x24; j += 4, size++) {
			*size = PCI_findBARSize(j,&info[i]);
			}
		info[i].ROMBaseAddressLen = PCI_findBARSize(0x30,&info[i]);
		}
	return numFound;
}
	...
	uint IRQ)
{
	if (PCIPhysEntry) {
		if (_PCIBIOS_setIRQ(info->slot.i >> 8,intPin,IRQ,PCIEntry)) {
			info->InterruptPin = intPin;
			info->InterruptLine = IRQ;
			return true;
			}
		return false;
		}
