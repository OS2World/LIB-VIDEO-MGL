...
@@NotIntel:
		cmp		[DWORD cyrix_id], ebx
		jne		@@NotCyrix
		cmp		[DWORD cyrix_id+4], edx
		jne		@@NotCyrix
		cmp		[DWORD cyrix_id+8], ecx
		jne		@@NotCyrix
...
@@NotCyrix:
		cmp		[DWORD amd_id], ebx
		jne		@@NotAMD
		cmp		[DWORD amd_id+4], edx
		jne		@@NotAMD
		cmp		[DWORD amd_id+8], ecx
		jne		@@NotAMD
...
@@NotAMD:
		cmp		[DWORD idt_id], ebx
		jne		@@NotIDT
		cmp		[DWORD idt_id+4], edx
		jne		@@NotIDT
		cmp		[DWORD idt_id+8], ecx
		jne		@@NotIDT
...
cprocstart	_CPU_getCacheSize
		...
		lea		esi,[cache_id]		; Get address of cache ID
		shr		eax,8
		mov		[esi+0],eax
		mov		[esi+3],ebx
		mov		[esi+7],ecx
...
;----------------------------------------------------------------------------
; void _CPU_outp(int port,int value)
;----------------------------------------------------------------------------
; Write a byte to the specified port.
;----------------------------------------------------------------------------
cprocstart	_CPU_outp
