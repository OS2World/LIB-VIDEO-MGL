...
int 	dither = GL_TRUE, fog = GL_FALSE;
...
void openXinput(void);
void closeXinput(void);
...
void cull_proc(void)
{
	...
	float           left_dx, left_dy, right_dx, right_dy, far_dx=0, far_dy=0;
	...
	for (x = x0; x < x1; x++) {
		for (y = y0; y < y1; y++) {
			...
			if ((left_area < cull.epsilon * cull.side && right_area > -cull.epsilon * cull.side
				/*&& far_area < cull.epsilon * cull.far*/) || 0) {
	...
	*/
	for (i = 0; i < NUM_PLANES; i++) {
		...
		if (left_area < cull.plane_epsilon * cull.side && right_area > -cull.plane_epsilon * cull.side
			/*&& far_area < cull.plane_epsilon * cull.far*/) {
...
void calcStereoMatrix(void)
{
	...
	glGetFloatv(GL_PROJECTION_MATRIX, &mProjectionLeft);
	glPopMatrix();
	...
	glTranslatef(-fDxViewpoint, 0.0, 0.0);
	glGetFloatv(GL_PROJECTION_MATRIX, &mProjectionRight);
	glPopMatrix();
...
void init_gl(int gfxpipenum)
{
	...
	gluPerspective(FOV * RAD_TO_DEG, (float)Wxsize/(float)Wysize,
														 .1, far_cull *.95);
#ifdef  STEREO
	/* remember frustum settings for stereo3d perspective adjustments */
	fFrustumZnear = 0.1;
	fFrustumZfar = far_cull * 0.95;
	/* gluPerspective() trigonometry: tan(FOVangle/2) = (Yheight/2) / Znear */
	fFrustumHeight2 = tan(FOV/2.0) * fFrustumZnear;
	fFrustumWidth2 = fFrustumHeight2 * (GLfloat)Wxsize/(GLfloat)Wysize;
	/* estimate frustum shift = approx 5% of frustum width */
	fDxImageShift = 0.05 * fFrustumWidth2;
	/* set left/right offset from default perspective viewpoint */
	fDxViewpoint = 0.10;
	calcStereoMatrix();
#endif
...
void err_msg(int type, char* func, char* error)
{
	...
	if (type & ERR_FATAL) {
		// TODO: This should be an MGL_exit() first
		exit(-1);
		}
}
