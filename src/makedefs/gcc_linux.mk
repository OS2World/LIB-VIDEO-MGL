...
#       the .x extension.
   L            := .a   	# Libraries
.IF $(LIBC)
   E            := .5x   	# Executables for libc5
.ELSE
   E            := .x   	# Executables for glibc
.ENDIF
   O            := .o   	# Objects
...
.IF $(LIBC)
   LIB_DEST     := $(LIB_BASE_DIR)/linux/gcc/libc
.ELSE
   LIB_DEST     := $(LIB_BASE_DIR)/linux/gcc/glibc
.ENDIF
   LDFLAGS      += -L$(LIB_DEST)

# Optionnally enable some  dynamic libraries to be built
.IF $(DYNAMIC_LIBS)
   VERSIONMAJ	:= 4
   VERSIONMIN	:= 5
